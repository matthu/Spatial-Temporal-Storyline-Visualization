...
export function scale(story: Story, constraints: any[]) {
  ...
  for (let i = 0, n = story.getTableRows(); i < n; i++) {
    ...
    for (let j = 0, m = story.getTableCols(); j < m; j++) {
      pos[i][j] = []
      if (character?.value(i, j) && layout?.value(i, j) as number >= 0) {
        const storySegment = positions[position?.value(i, j) as number]
...
export function genNewPosition(story: Story, pos: number[][][][]) {
...
function freeTransform(story: Story, constraints: Constraint[]) {
  const pathTable = genPath(story, constraints)
  story.setTable('path', pathTable)
  return pathTable
}

export function genPath(story: Story, constraints: Constraint[]) {
  story.cleanPaths()
  // const position = story.getTable('position')
  ...
  for (let i = 0, n = story.getTableRows(); i < n; i++) {
    ...
    for (let j = 0, m = story.getTableCols(); j < m; j++) {
      if (character?.value(i, j) && layout?.value(i, j) as number >= 0) {
        const segment = positions[position?.value(i, j) as number]
